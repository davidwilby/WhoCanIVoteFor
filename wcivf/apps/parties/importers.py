...
from parties.models import GeneralElectionParty, LocalParty, Manifesto, Party

LocalElection = namedtuple("LocalElection", ["date", "csv_files"])
GeneralElection = namedtuple("GeneralElection", ["date", "csv_files"])
...
class LocalPartyImporter(ReadFromUrlMixin, ReadFromFileMixin):
    ...
    def add_manifesto(self, row, party, election, file_url):
        ...
        country = LocalPartyImporter.get_country(
...
class GeneralElectionPartyImporter(ReadFromUrlMixin, ReadFromFileMixin):
    JOINT_PARTIES = [["party:53", "joint-party:53-119"]]
    ...
    def delete_parties(self):
        """
        Deletes GeneralElectionParty objects associated with elections for the given
        election date
        """
        count, _ = GeneralElectionParty.objects.filter(
            file_url__in=self.election.csv_files,
        ).delete()
    ...
    def add_general_election_party(self, row, party, ballots, file_url):
        """
        Takes a row of data, a Party, and a QuerySet of at least one
        PostElection objects, and creates a GeneralElectionParty for each of the ballots.
        """
        ...
        for post_election in ballots.distinct():
            ...
            _, created = GeneralElectionParty.objects.update_or_create(
                parent=party, post_election=post_election, defaults=defaults
            )
            msg = "Created" if created else "Updated"
            self.write(
    ...
    def import_parties(self):
        """
        This is the main action that is run by the class. First existing
        GeneralElectionParty objects for an election are deleted. Then it validates the
        row contains data we can use. Then adds a GeneralElectionParty object for each
        parent party.
        """
        self.delete_parties()
        self.delete_manifestos()
        ...
        for file_url in self.election.csv_files:
            ...
            for row in rows:
                ...
                # add_general_election_party method has been called, None is
                ...
                for party in parties:
                    self.add_general_election_party(
    ...
    def add_manifesto(self, row, party, election, file_url):
        manifesto_web = row.get("Manifesto Website URL", "").strip()
        manifesto_pdf = row.get("Manifesto PDF URL", "").strip()
        country = GeneralElectionPartyImporter.get_country(
            election_type=election.election_type
        )
        language = row.get("Manifesto Language", "English").strip()
        easy_read_url = row.get("Manifesto Easy Read PDF", "").strip()
